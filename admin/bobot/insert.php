	...
	if ($totalRows_rs_cek == 0) {
		 do {
		  $insertSQL = sprintf("INSERT INTO tb_bobot (kriteria_id, alternatif_id) VALUES (%s, %s)",
							   GetSQLValueString($koneksi, $row_rs_kriteria2['id_kriteria'], "int"),
							   GetSQLValueString($koneksi, $colname_rs_alternatif, "int"));
							   
		  mysqli_select_db($koneksi, $database_koneksi);
		  $Result1 = mysqli_query($koneksi, $insertSQL) or die(mysqli_error($koneksi));
		} while ($row_rs_kriteria2 = mysqli_fetch_assoc($rs_kriteria2));
	}
}else{
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
                ...
                <script>
                  function nilai<?= $no;?>(){
                    var data = document.getElementById("combo1<?= $no;?>").value;
                    document.getElementById("nilai_<?= $no;?>").value=data;
                  }
                </script>

                <select class="form-control" name="combo" id="combo1<?= $no;?>" onchange="nilai<?= $no;?>()">
                  <option value="0">Pilih Bobot Penilaian</option>
                  <option value="100">Sangat Baik</option>
                  <option value="75">Cukup Baik</option>
                  <option value="50">Kurang Baik</option>
                  <option value="25">Tidak Baik</option>
                </select>

                <input id="nilai_<?= $no;?>" type="text" name="nilai_bobot<?= $no;?>" value="<?php echo $row_rs_bobot['nilai_bobot']; ?>" size="32">
